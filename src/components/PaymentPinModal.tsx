...
export const PaymentPinModal: React.FC<PaymentPinModalProps> = ({
  ...
  const order = orderSnapshot.val();
    ...
    if (!transactionKey) {
      throw new Error('無法建立交易編號');
    }

    // 5. 同時修改三個地方
    await update(ref(database), {
      [`users/${userId}/balance`]: balanceAfter,

      [`orders/${orderId}/status`]: 'PAID',
      [`orders/${orderId}/paid_at`]: now,

      [`transactions/${userId}/${transactionKey}`]: {
        user_id: userId,
        order_id: orderId,
        amount: amount,
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-sm w-full p-6 shadow-2xl border border-slate-200 dark:border-slate-800 animate-in fade-in zoom-in-95 duration-200">
        <div className="flex justify-between items-center pb-4 border-b border-slate-100 dark:border-slate-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-slate-900 dark:text-white">輸入支付 PIN 碼</h3>
              <p className="text-xs text-slate-500">掃碼授權支付扣款</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handlePay} className="mt-5 space-y-4">
          <div className="p-3 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-200 dark:border-slate-700 text-xs text-slate-600 dark:text-slate-300 flex items-center gap-2">
            ...
            <span>目前偵測到之 QR Token：<code className="font-mono text-[10px] bg-slate-200 dark:bg-slate-700 px-1 py-0.5 rounded">{qrCodeToken.slice(0, 22)}...</code></span>
          </div>
